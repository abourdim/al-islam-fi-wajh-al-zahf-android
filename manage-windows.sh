#!/usr/bin/env bash
# ============================================================================
#  Al Islam fi Wajh al-Zahf Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/08-al-islam-fi-wajh-al-zahf}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alislamfiwajhalzahf}"
AVD_NAME="${AVD_NAME:-al_islam_fi_wajh_al_zahf_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-islam-fi-wajh-al-zahf-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Islam fi Wajh al-Zahf Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
