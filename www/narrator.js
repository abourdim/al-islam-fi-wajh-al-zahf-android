...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','cards','contrasts'],
    tabIndex: 0,
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'الإسلام في وجه الزحف الأحمر' : l === 'fr' ? 'L\'Islam Face a la Maree Rouge' : 'Islam Facing the Red Tide',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('zahf-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('zahf-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('zahf-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('zahf-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('zahf-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('zahf-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('zahf-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('zahf-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('zahf-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('zahf-narrator-duo') === 'true';
  }
  ...
  var origRenderContrasts = window.renderContrasts;
  if (origRenderContrasts) {
    window.renderContrasts = function() { origRenderContrasts(); setTimeout(injectSpeakButtons, 100); };
  }
